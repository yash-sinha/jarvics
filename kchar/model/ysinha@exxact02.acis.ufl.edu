...
from keras.layers import Input, Embedding, TimeDistributed, Dense, Dropout, Reshape, Concatenate, LSTM, Conv2D, MaxPooling2D, BatchNormalization, Conv1D
...
def CNN(seq_length, length, input_size, feature_maps, kernels, x):
    ...
    for feature_map, kernel in zip(feature_maps, kernels):
        ...
        conv1 = Conv2D(feature_map, (1, kernel), strides=1, activation='tanh', data_format="channels_last")(x)

        # Layer 2: Conv2D layer with `squash` activation, then reshape to [None, num_capsule, dim_vector]
        primarycaps = PrimaryCap(conv1, dim_vector=feature_map, n_channels=32, kernel_size=(1, kernel), strides=2, padding='valid', filters = feature_map)

        # Layer 3: Capsule layer. Routing algorithm works here.
        digitcaps = CapsuleLayer(num_capsule=seq_length, dim_vector=feature_map, num_routing=3)(
            primarycaps)
...
def LSTMCNN(opt):
    ...
    for l in range(opt.num_layers):
        x = LSTM(opt.rnn_size, activation='tanh', recurrent_activation='sigmoid', return_sequences=True, stateful=False)(x)
